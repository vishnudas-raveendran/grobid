...
public class ProcessEngine implements Closeable {
	...
	if (files == null) {
		...
		elementCoordinates = Arrays.asList("figure", "persName", "ref", "biblStruct", "formula", "Text");
	    }
    ...
    private void processFullTextDirectory(File[] files, final GrobidMainArgs pGbdArgs, String outputPath,
	    boolean saveAssets, List<String> elementCoordinates) {
	if (files != null) {
	    boolean recurse = pGbdArgs.isRecursive();
	    String result;
	    for (final File currPdf : files) {
		try {
			...
			if (saveAssets) {
			    ...
			    config = GrobidAnalysisConfig.builder().pdfAssetPath(new File(assetPath))
				    .generateTeiCoordinates(elementCoordinates).build();
			} else
			...
			if (newFiles != null) {
			    String newLevel = currPdf.getName();
			    processFullTextDirectory(newFiles, pGbdArgs, outputPath + File.separator + newLevel,
				    saveAssets, elementCoordinates);
			}
		    }
		} catch (final Exception exp) {
		    LOGGER.error("An error occured while processing the file " + currPdf.getAbsolutePath()
	...
	if (files != null) {
		...
		try {
			...
			if (currPdf.getName().endsWith(".pdf")) {
			    IOUtilities.writeInFile(outputPath + File.separator + new File(currPdf.getAbsolutePath())
				    .getName().replace(".pdf", ".references.tei.xml"), result.toString());
			} else if (currPdf.getName().endsWith(".PDF")) {
			    IOUtilities.writeInFile(outputPath + File.separator + new File(currPdf.getAbsolutePath())
				    .getName().replace(".PDF", ".references.tei.xml"), result.toString());
			}
    ...
    /**
     * Generate training data for the monograph model from provided directory of PDF
     * documents.
     *
     * @param pGbdArgs The parameters.
     * @throws Exception
     ...
     * where TEI files are text only without tags. This can be used to start from
     * scratch any new model.
     *
     * @param pGbdArgs The parameters.
     * @throws Exception
     */
	...
	inferPdfInputPath(pGbdArgs);
	inferOutputPath(pGbdArgs);
	int result = getEngine().batchCreateTrainingPatentcitations(pGbdArgs.getPath2Input(),
     ...
     */
    /*
     * public void processCitationPatentTEI(final GrobidMainArgs pGbdArgs) throws
     * Exception { inferPdfInputPath(pGbdArgs); inferOutputPath(pGbdArgs);
     * 
     * final File teiDirectory = new File(pGbdArgs.getPath2Input()); String result =
     * StringUtils.EMPTY; for (final File currTEI : teiDirectory.listFiles()) { try
     * { if (currTEI.getName().toLowerCase().endsWith(".tei") ||
     * currTEI.getName().toLowerCase().endsWith(".tei.xml")) {
     * getEngine().processCitationPatentTEI(pGbdArgs.getPath2Input() +
     * File.separator + currTEI.getName(), pGbdArgs.getPath2Output() +
     * File.separator + currTEI.getName(), 0); } } catch (final Exception exp) {
     * LOGGER.error("An error occured while processing the file " +
     * currTEI.getAbsolutePath() + ". Continuing the process for the other files",
     * exp); } } }
     */
	...
	for (final File currXML : xmlDirectory.listFiles()) {
		...
		if (currXML.getName().toLowerCase().endsWith(".xml")
		    ...
		    result = getEngine().processAllCitationsInXMLPatent(
			    pGbdArgs.getPath2Input() + File.separator + currXML.getName(), articles, patents, 0, false);
	...
	for (final File currPDF : pdfDirectory.listFiles()) {
		...
		if (currPDF.getName().toLowerCase().endsWith(".pdf")) {
		    ...
		    result = getEngine().processAllCitationsInPDFPatent(
			    pGbdArgs.getPath2Input() + File.separator + currPDF.getName(), articles, patents, 0, false);
		    if (currPDF.getName().endsWith(".pdf")) {
			IOUtilities.writeInFile(
				pGbdArgs.getPath2Output() + File.separator
					+ new File(currPDF.getAbsolutePath()).getName().replace(".pdf", ".tei.xml"),
	...
	for (final File currPDF : pdfDirectory.listFiles()) {
		...
		if (currPDF.getName().toLowerCase().endsWith(".pdf")) {
		    ...
		    GrobidAnalysisConfig config = new GrobidAnalysisConfig.GrobidAnalysisConfigBuilder()
			    .consolidateCitations(1).generateTeiCoordinates(elementWithCoords).build();
